...
func main() {
    ...
    if cfgPath != "" {
        ...
        if err != nil { log.Fatalf("config read: %v", err) }
        if err := yaml.Unmarshal(b, &cfg); err != nil { log.Fatalf("config parse: %v", err) }
    }
    if paths != "" {
        cfg.Roots = nil
        for _, p := range strings.Split(paths, ",") {
            p = strings.TrimSpace(p)
            if p != "" { cfg.Roots = append(cfg.Roots, p) }
        }
    }
    if debounce > 0 { cfg.DebounceMillis = debounce }
        ...
        for ev := range w.Events() {
            ...
            if !ev.IsDir {
                ...
                if err != nil {
                    ...
                    if store != nil { _ = store.InsertParseError(context.Background(), ev.Path, err.Error(), 0) }
                    continue
                }
                // Fakten persistieren
                ...
                if store != nil {
                    _ = store.UpsertFile(ctx, storage.FileMeta{Path: ev.Path, Ext: ext, Size: ev.Size, MTime: ev.MTime})
                    _ = store.ClearFactsForFile(ctx, ev.Path)
                    for _, s := range facts.Symbols { _ = store.InsertSymbol(ctx, ev.Path, s.Kind, s.Name, s.LineStart, s.LineEnd, s.Meta) }
                    for _, d := range facts.Deps { _ = store.InsertDep(ctx, ev.Path, d.Name, d.Line, d.Meta) }
                    for _, a := range facts.APIs { _ = store.InsertAPI(ctx, ev.Path, a.Framework, a.Method, a.Route, a.Line, a.Meta) }
                    for _, st := range facts.Strings { _ = store.InsertString(ctx, ev.Path, st.Line, st.Text, st.Meta) }
                    for _, cmt := range facts.Comments { _ = store.InsertComment(ctx, ev.Path, cmt.Line, cmt.Text, cmt.Meta) }
                }
            }
        }
    }()
