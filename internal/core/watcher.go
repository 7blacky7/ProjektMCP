...
func (w *Watcher) Start() error {
	...
	for _, r := range w.cfg.Roots {
		if err := w.addRecursive(r); err != nil {
			errs = append(errs, fmt.Errorf("root %s: %w", r, err))
		}
	...
	walkErr := filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		...
		if info.IsDir() {
			...
			if err := w.fsw.Add(path); err != nil {
				errs = append(errs, fmt.Errorf("watch add %s: %w", path, err))
			}
		}
		return nil
	})
...
func (w *Watcher) shouldIgnore(path string) bool {
	rel := path
	if len(w.cfg.Roots) > 0 {
		if r, err := filepath.Rel(w.cfg.Roots[0], path); err == nil {
			rel = r
		}
	}
...
func matchPattern(filePath, pattern string) bool {
	...
	if strings.ContainsAny(pattern, "*?") {
		...
		if ok, _ := filepath.Match(simp, filePath); ok {
			return true
		}
		return false
	}
